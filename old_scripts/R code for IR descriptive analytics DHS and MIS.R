...
####and subdirectories in the working directory
...
bf10dhs <- `BF_2010_DHS_06192019/BFIR62DT/BFIR62FL` #2010 DHS 
bf03dhs <- `BF_2003_DHS_06192019/BFIR43DT/BFIR43FL` #2003 DHS 
bf98dhs <- `BF_1998-99_DHS_06192019/BFIR31DT/BFIR31FL` #1998 DHS 
bf93dhs <-`BF_1993_DHS_06192019/BFIR21DT/BFIR21FL` #1993 DHS 
...
pts_10_merge<-data.frame(v001=pts_10@data$DHSCLUST, DHSCLUST=pts_10@data$DHSCLUST) #this is dhs 14 
...
####the dimensions of the pts data is 573 by 21 and of the DS is 70 by 11, key_10 will be 573 by 10 
...
fun0(bf17mis, 'b2_01', 'ml25a_1','ml0_1', 's405a', 's405b', 'h22_1', 'h32z_1', 'ml13e_1', 'm49a', 'v459', 'ml101', 'h46b_1', 'v213')
fun0(bf14mis, 'b2_01', 'ml0_1', 's405a', 's405b', 'h22_1', 'h32z_1', 'ml13e_1', 'v024', 's307a', 's127', 's129', 's307d', 'v213')
fun0(bf10dhs, 'b2_01', 'ml0_1', 's405a', 's405b', 'h22_1', 'h32z_1', 'ml13e_1', 'v024', 's307a', 's127', 's129', 's307d', 'v213')
...
bf10dhs$MM <- bf10dhs$v008 - ((bf10dhs$v007 - 1900) * 12)


####Applying the function to each dataset 
###We start with the 2017 and 2014 BF MIS dataset
mis17 <- fun1(bf17mis, 'b2', 's111','ml25a', 'ml20a', 'ml0', 'h22', 'h32z', 'h46a', 'ml13e','ml13a','m49a','v459','ml101','ml2','MM', 'v007', 'v213') 
mis14 <- fun1(bf14mis, 'b2', 'ml0', 'h22', 'h32z', 'ml13e', 's127', 's307','MM', 'v007', 's129', 'v213')
dhs10 <- fun1(bf10dhs, 'b2', 'ml0', 'h22', 'h32z', 'ml13e', 'v213')
dhs93 <- fun1(bf93dhs, 'h22', 'h32', 'h33a')
...
view(dfSummary(dhs10))
view(dfSummary(dhs93))
...
summary(bf10dhs$v001)# the cluster variable
summary(key_10$v001)
...
####################################################################################
#### selecting only LLIN variables for analysis and other data processing for 2017 BF MIS 
...
summary(pt_n$`U5 in DHS clusters`)
#this is for 2017 BF 
...
st_write(pt_f, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/Fever_2017/fever_2017_cluster.shp")
st_write(pt_f14, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/Fever_2014/fever_2014_cluster.shp")
...
st_write(pt_m, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/Medical_Treat_2017/shape/medtreat_2017_cluster.shp")
st_write(pt_m14, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/Medical_Treat_2014/shape/medtreat_2014_cluster.shp")
...
summary(pt_act14$act)

st_write(pt_act, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/ACT_fever_2017/act_17.shp")
st_write(pt_act14, "C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/ACT_fever_2014/act_14.shp")
...
st_write(pt_iptp17,"C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/IPTP_spf_2017/iptp17.shp")
st_write(pt_iptp,"C:/Users/ido0493/Box/NU-malaria-team/projects/hbhi_burkina/cluster_values/IPTP_spf_2014/iptp14.shp")
...
#####################################################################################################
#---Estimating the proportion of pregnant women 15 - 49 years by DS---#
#####################################################################################################

#####################################################################################################
# -- We start by setting up the design variables -- #
####################################################################################################
#### This is for MIS 2017  
mis17_v213<-mis17_r %>%
  mutate(wt=v005/1000000,strat=v022,
         id=v021, num_f=1)%>%
  rename(pregnant = v213) %>%
  filter(!is.na(wt))
  



V213.svydesign_17 <- svydesign(id= ~id,
                          strata=~strat,nest=T, 
                          weights= ~wt, data=mis17_v213)

####This is for MIS 2014

mis14_v213<-mis14_r%>%mutate(wt=v005/1000000,strat=v022,
                          id=v021, num_f=1)%>%
                   rename(pregnant = v213) %>%
                   filter(!is.na(wt))
                     

V213.svydesign_14 <- svydesign(id= ~id,
                               strata=~strat,nest=T, 
                               weights= ~wt, data=mis14_v213)


####This is for DHS 2010 

dhs10_v213 <- bf10dhs_work%>% mutate(wt=v005/1000000,strat=v022,
                                      id=v021, num_f=1)%>%
                            rename(pregnant = v213) %>%
                            filter(!is.na(wt))
                               
V213.svydesign_10 <- svydesign(id= ~id,
                               strata=~strat,nest=T, 
                               weights= ~wt, data=dhs10_v213)


#####################################################################################################
# -- Proportion of women that pregnant women by health district -- #
#####################################################################################################
####This calculates the proportion of pregnant women by health district for 2017 BF MIS 
system.time({
  v213_17<-svyby(formula=~pregnant,# which variable do we want estimate
                 by=~NOMDEP, # by which variable
                 FUN=svymean, # what is the function (svymean gives the horvitz thompson estimate and variance)
                 design=V213.svydesign_17, # my svy design object
                 na.rm=T)
})

####This calculates the proportion of pregnant women by health district for 2014 BF MIS 
system.time({
  v213_14<-svyby(formula=~pregnant,# which variable do we want estimate
                 by=~NOMDEP, # by which variable
                 FUN=svymean, # what is the function (svymean gives the horvitz thompson estimate and variance)
                 design=V213.svydesign_14, # my svy design object
                 na.rm=T)
})


####This calculates the proportion of pregnant women by health district for 2010 BF DHS 
system.time({
  v213_10<-svyby(formula=~pregnant,# which variable do we want estimate
                 by=~NOMDEP, # by which variable
                 FUN=svymean, # what is the function (svymean gives the horvitz thompson estimate and variance)
                 design=V213.svydesign_10, # my svy design object
                 na.rm=T)
})


######################################################################################################
# -- Number of women in the each health district -- #
####################################################################################################
####This computes the total number of women within each surveyed cluster for this variable 2017 BF MIS 
system.time({
  v213_17_num<-svyby(formula=~num_f,# which variable do we want estimate
                        by=~NOMDEP, # by which variable
                        FUN=svytotal, # what is the function (svytotal estimates totals and variance from complex surveys)
                        design=V213.svydesign_17, # my svy design object
                        na.rm=T)
...
####This computes the total number of women within each surveyed cluster for this variable 2014 BF MIS 
system.time({
  v213_14_num<-svyby(formula=~num_f,# which variable do we want estimate
                        by=~NOMDEP, # by which variable
                        FUN=svytotal, # what is the function (svytotal estimates totals and variance from complex surveys)
                        design=V213.svydesign_14, # my svy design object
                        na.rm=T)
...
####This computes the total number of women within each surveyed cluster for this variable 2010 BF MIS 
system.time({
  v213_10_num<-svyby(formula=~num_f,# which variable do we want estimate
                         by=~NOMDEP, # by which variable
                         FUN=svytotal, # what is the function (svytotal estimates totals and variance from complex surveys)
                         design=V213.svydesign_10, # my svy design object
                         na.rm=T)
...
####we want to merge with the full list of health districts
#we start with the proportion of women are pregnant by health district for the 2017 BF MIS 
dim(DS_merge)
DS_merge_v213_17 <-DS_merge%>%left_join(v213_17) #for 2017
DS_merge_v213_14 <-DS_merge%>%left_join(v213_14) #for 2014 
DS_merge_v213_10 <-DS_merge%>%left_join(v213_10) #for 2010 

####we also want to merge the full list of health districts with the number of women dataframe 
DS_merge_num_17<-DS_merge%>%left_join(v213_17_num) #for 2017
DS_merge_num_14<-DS_merge%>%left_join(v213_14_num) #for 2014 
DS_merge_num10<-DS_merge%>%left_join(v213_10_num) #for 2010 

####Changing column names in preparation of merging the DS estimates of pregnant women with the number of women 
colnames(DS_merge_num_17)[3]<- "standard_error" #for 2017
colnames(DS_merge_num_14)[3]<- "standard_error" #for 2014
colnames(DS_merge_num10)[3]<- "standard_error" #for 2010

####rounding the number of women in each year's dataset 
DS_merge_num_17$num_f <- round(DS_merge_num_17$num_f, 0)
DS_merge_num_14$num_f <- round(DS_merge_num_14$num_f, 0)
DS_merge_num10$num_f <- round(DS_merge_num10$num_f, 0)

####Final dataset with DS level estimates of pregnant women
pregnant_BF_17 <- DS_merge_v213_17 %>%left_join(DS_merge_num_17)
pregnant_BF_14 <- DS_merge_v213_14 %>% left_join(DS_merge_num_14) 
pregnant_BF_10 <- DS_merge_v213_10 %>% left_join(DS_merge_num10) 

summary(pregnant_BF_17$pregnant)
summary(pregnant_BF_14$pregnant)
summary(pregnant_BF_10$pregnant)

####creates a csv file of the proportion of children with fever by health district 
write.csv(pregnant_BF_17 ,file="outputs/pregnant_BF17DS.csv") #for the 2017 BF MIS
write.csv(pregnant_BF_14,file="outputs/pregnant_BF14DS.csv") #for the 2014 BF MIS
write.csv(pregnant_BF_10,file="outputs/pregnant_BF10DS.csv") #for the 2014 BF MIS