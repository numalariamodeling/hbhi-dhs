...
in_files_I <- list.files(pattern = ".*NGIR.*\\.DTA", recursive = TRUE)
in_files_H <- list.files(pattern = ".*NGHR.*\\.DTA", recursive = TRUE)
in_files_P <- list.files(pattern = ".*NGPR.*\\.DTA", recursive = TRUE)

filenames <- rbind(in_files_I, in_files_H, in_files_P)
NGAfiles <- sapply(filenames, read_dta, simplify = F)
...
#all_month_list <- map(month_ptest_10, right_join, rownames_to_column(DSshape_sf))
#list2env(all_month_list ,.GlobalEnv)
#jan <-st_as_sf(`1-2010`)
...
ptest_month_14_map<- tmap_arrange(ptest_2014_sep_map, ptest_2014_oct_map, ptest_2014_nov_map, 
                               ptest_2014_dec_map)
...
#saving map
tmap_save(tm = ptest_month_map_10, filename = "outputs/ptest_month_map_2010.pdf", 
          width=13, height=13, units ="in", asp=0,paper ="A4r", useDingbats=FALSE)
...
colnames(IRS_clu_10)[3]<- "standard_error" #2010 HR BF DHS

#renaming 'se' colname in IRS_clu_14 to standard error in preparation for the joining procedure 
colnames(IRS_clu_14)[3]<- "standard_error" #2014 HR BF MIS


#merging the DS level estimates with the full list of health districts
DS_IRS_10<-DSshape_sf%>%left_join(IRS)%>%left_join(num_DS_10) %>% #2010 HR BF DHS
                 rename(`Number of HH` = num_HH)    
head(DS_IRS_10)

DS_IRS_14 <- DSshape_sf%>%left_join(IRS_14)%>%left_join(num_DS_14) %>%  #2014 HR BF MIS
                 rename(`Number of HH` = num_HH) 

head(DS_IRS_14)

#cluster estimates are merged with the cluster shapefile 
pts_est_IRS<- pt_sf_10 %>% left_join(IRS_clu_10)%>%left_join(num_clu_10) %>% #2010 HR BF DHS
                rename(`HH in DHS clusters` = num_HH) %>% 
                rename(`HH sprayed with IRS by cluster`= IRS)

head(pts_est_IRS)

pts_est_IRS_14<-pt_sf_14%>%left_join(IRS_clu_14)%>%left_join(num_clu_14) %>%  #2014 HR BF MIS
                         rename(`HH in DHS clusters` = num_HH) %>% 
                          rename(`HH sprayed with IRS by cluster`= IRS)
head(pts_est_IRS_14)
...
#2010 HR BF DHS
IRS_2010_map <- tm_shape(DS_IRS_10) + #this is the health district shapfile with test result info
  tm_polygons(col = "IRS", textNA = "No data", 
              title = "Proportion of HH that received IRS", palette = "RdYlBu")+
...
#2014 HR BF MIS
IRS_2014_map <- tm_shape(DS_IRS_14) + #this is the health district shapfile with test result info
  tm_polygons(col = "IRS", textNA = "No data", 
              title = "Proportion of HH that received IRS", palette = "RdYlBu")+
  tm_layout(main.title="Proportion of HH that received IRS by Health Districts (2014)",
            main.title.position = c("center", "top"))+
  tm_text("NOMDEP", size=0.5, root = 4)
