...
smc_scen_3 <- read.csv('bin/projection/s3_v3/smc_PAAR.csv') %>% dplyr::rename(LGA = adm2)
smc_scen_3$LGA <- gsub("\\/", "-", smc_scen_3$LGA)
head(smc_scen_3)
...
smc_3 <- smc_scen_3 %>%mutate(LGA = str_to_title(LGA)) %>% left_join(rep_DS) 
...
repDS <- unique(as.character(smc_3$repDS))
repDS


peak <- c('july', 'july', 'may', 'august', 'july', 'september', 'august', 'july', 'july', 'june', 'august', 'march', 'august',
          'march', 'august')

df <- tibble(repDS, peak)


smc_3 <- smc_3 %>% left_join(df)
head(smc_3)
...
#2022
smc_2022 <- smc_2021 %>% mutate(simday = if_else(peak == 'july',915,if_else(peak == 'may',852,
...
#2023 
smc_2023 <- smc_2021 %>% mutate(simday = if_else(peak == 'july',1286,if_else(peak == 'may',1223,
...
#2024 
smc_2024 <- smc_2021 %>% mutate(simday = if_else(peak == 'july',1646,if_else(peak == 'may',1587,
...
#2025 
smc_2025 <- smc_2021 %>% mutate(simday = if_else(peak == 'july',2014,if_else(peak == 'may',1951,
