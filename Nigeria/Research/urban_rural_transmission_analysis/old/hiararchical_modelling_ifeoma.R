...
DHSDir <- file.path(NGDir, "data", 'DHS')
CluDir <- file.path(NGDir, "data", 'DHS', 'Computed_cluster_information')
UrbanClu <- file.path(NGDir, "data", 'DHS', 'Computed_cluster_information', 'urban_malaria_covariates')
...
PubDir <- file.path(ProjectDir, 'figures')
PrintDir <- file.path(ProjectDir, 'Illustrations')
...
# make state cluster maps to see where low transmission clusters are located 
...
dhs18_sf <- st_read(file.path(DHSDir, 'Downloads', "NG_2018_DHS_11072019_1720_86355/NGGE7BFL/NGGE7BFL.shp"),) %>%   filter(URBAN_RURA == 'U')
mis15_sf <- st_read(file.path(DHSDir,'Downloads', "NG_2015_MIS_06192019/NGGE71FL/NGGE71FL.shp"),) %>%  filter(URBAN_RURA == 'U')
mis10_sf <- st_read(file.path(DHSDir,'Downloads', "NG_2010_MIS_06192019/NGGE61FL/NGGE61FL.shp"),)%>%  filter(URBAN_RURA == 'U')
...
map_18<-clustermap(dhs_18, "2018 malaria prevalence by cluster (DHS)")
map_15 <-clustermap(dhs_15, "2015 malaria prevalence by cluster (DHS)")
map_10 <-clustermap(dhs_10, "2010 malaria prevalence by cluster (DHS)")
...
y <- ifelse(urbandataset$p_test < 0.1, 0, 1) 
covariate_df <- urbandataset %>%select(-c(hv001, data_source))

result <- lapply(covariate_df[1:22], function(x){glm(y ~ 1 + x, family = 'binomial')})#fits the model for all columns 





w_glm <- glm(urbandataset$y~ 1+ urbandataset$wealth_2, family = "binomial")
...
u_rmod <- inla(y ~ 1+ wealth_2 + edu_a + net_use + hh_size + ACT_use_u5 +
                 hh_members_age + sex_f + log(annual_precipitation) +log(build_count) + humidindex + interview_month, family = 'binomial',
...
u_iid <- inla(y ~ 1+ wealth_2 + edu_a + net_use + hh_size + ACT_use_u5 +
                hh_members_age + sex_f + log(annual_precipitation) +log(build_count)+  humidindex+ interview_month +f(state, model = "iid"), family = 'binomial',
              data = urbandataset, control.family = list(link = "logit"), control.predictor = list(compute=TRUE),
              control.compute = list(cpo=TRUE, dic = TRUE))
...
u_map <- tm_shape(u_map_df)+
  tm_polygons(col = "state.mean", midpoint =NA, palette = "-RdYlGn", breaks =c(-2, -1.0, -0.5, 0, 0.2, 1.0, 1.5, 2.0))+
  tm_text("NAME_1")
...
#read in pfpr data 
lag_pfpr <- read.csv(file.path(UrbanClu, 'p_test_lagos_PfPR_DHS_10_15_18.csv')) %>% 
  mutate(year = str_split(.id, "_", simplify = T)[, 4])

lag_year = split(lag_pfpr, lag_pfpr$year)

#join cluster shp to dhs cluster pfpr data
clu_10 <- mis10_sf  %>% filter(ADM1NAME == 'LAGOS') %>%  left_join(lag_year[[1]], by =c('DHSCLUST' = 'hv001'))
clu_15 <- mis15_sf  %>% filter(ADM1NAME == 'Lagos') %>%  left_join(lag_year[[2]], by =c('DHSCLUST' = 'hv001'))
clu_18 <- dhs18_sf  %>% filter(ADM1NAME == 'LAGOS') %>%  left_join(lag_year[[3]], by =c('DHSCLUST' = 'hv001'))
...
dat_lag_malaria =
  ...
  tm_bubbles(size =0.2, col = "p_test", 
             border.col= "black", palette="seq",textNA = "Missing",
             breaks=c(0, 0.01, 0.05,0.1, 0.15, 0.20, 0.30, 0.35), legend.col.show=T)+
  tm_shape(clu_15)+ 
  tm_bubbles(size =0.2, col = "p_test", 
             border.col= "black", palette="seq",textNA = "Missing",
             breaks=c(0, 0.01, 0.05,0.1, 0.15, 0.20, 0.30, 0.35), legend.col.show=T)+
...
tmap_save(tm = dat_lag_malaria, filename =paste0(PubDir, '/', Sys.Date(),  '_lagos_pfpr_DHS.pdf')
          , width=13, height=13, units ="in", asp=0,
          paper ="A4r", useDingbats=FALSE)
...
df_18_fin <- left_join(dhs18_sf, dhs_18, by = c("DHSCLUST" ="hv001"))
...
#write_csv(key_list[[5]], file.path(DataDir, '2018_DHS_ID_and_LGA.csv'))
