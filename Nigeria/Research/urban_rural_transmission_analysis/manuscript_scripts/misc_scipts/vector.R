...
files <- list.files(path = file.path(RastDir , "vector") ,pattern = "*GA.tiff$", 
                    full.names = TRUE, recursive = TRUE)
files<- files[(grep('Global_5k', files))]
...
for (i in 1:length(vars)) {
  ...
  df <- plyr::ldply(df)
  write.csv(df, file = file.path(DataIn, paste0('Dominant_vector_', as.character(vars[i]), 'm_buffer', "_DHS_18.csv")))
}
...
for (i in 1:length(vars)) {
  ...
  df <- pmap(list(raster, df, vars[i]), extract_fun)
  df <- plyr::ldply(df)
  write.csv(df, file = file.path(DataIn, paste0('Dominant_vector_', as.character(vars[i]), 'm_buffer', "_DHS_15.csv")))  
} 
...
for (i in 1:length(vars)) {
  ...
  df <- pmap(list(raster, df, vars[i]), extract_fun)
  df <- plyr::ldply(df)
  write.csv(df, file = file.path(DataIn, paste0('secondary_vector_', as.character(vars[i]), 'm_buffer', "_DHS_10.csv")))  
} 
...
for (i in 1:length(vars)) {
  ...
  df <- pmap(list(raster, df, vars[i]), extract_fun)
  df <- plyr::ldply(df)
  write.csv(df, file = file.path(DataIn, paste0('secondary_vector_', as.character(vars[i]), 'm_buffer', "_DHS_18.csv")))
}
...
for (i in 1:length(vars)) {
  ...
  df <- pmap(list(raster, df, vars[i]), extract_fun)
  df <- plyr::ldply(df)
  write.csv(df, file = file.path(DataIn, paste0('secondary_vector_', as.character(vars[i]), 'm_buffer', "_DHS_15.csv")))  
} 
