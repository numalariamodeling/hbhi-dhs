...
model3 <- glm(y ~ edu_a + wealth_2, data = dat2, binomial) 
...
model1use <- glm(y ~ edu_a + wealth_2 +  net_use +  annual_precipitation + sex_f + hh_size + log_pop_den  + hh_members_age
                 + ACT_use_u5 + humidindex, data = dat2, binomial)
...
model1w <- glm(y ~ edu_a + wealth_2  + annual_precipitation + sex_f + hh_size + log_pop_den  + hh_members_age
...
model1b <- glm(y ~ edu_a + wealth_2 + log_pop_den + hh_size + sex_f + annual_precipitation + hh_members_age
               + net_use + ACT_use_u5 + humidindex, data = dat2, binomial)
...
model1c <- glm(y ~ edu_a + wealth_2 + hh_members_age + log_pop_den + hh_size + sex_f + annual_precipitation
               + net_use + ACT_use_u5 + humidindex, data = dat2, binomial)
...
model1f <- glm(y ~ edu_a  + wealth_2 + ACT_use_u5 + sex_f  + annual_precipitation + net_use + hh_members_age + log_pop_den + hh_size
...
model1g <- glm(y ~ edu_a  + wealth_2 + humidindex + ACT_use_u5 + sex_f  + annual_precipitation + net_use + 
                 hh_members_age + log_pop_den + hh_size, data = dat2, binomial)
...
dat3 <- dat2[,c("wealth_2", "edu_a", "build_count",
                ...
                "humidindex","annual_precipitation", "net_use")]
