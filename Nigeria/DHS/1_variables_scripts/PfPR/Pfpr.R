...
if (Variable == "PfPR"){
  ...
  pfpr.ls <- pfpr.ls[-c(1, 2, 3, 5)]
  pfpr.ls[[2]][,"hml32"] <-recoder.pfpr(pfpr.ls[[2]][,"hml32"]) # 2015 recoding pfpr (microscopy data)
  pfpr.ls[[1]][,"hml32"] <-recoder.pfpr(pfpr.ls[[1]][,"hml32"]) # 2010 recoding pfpr (microscopy data)
  table(pfpr.ls[[2]][,"hml32"])
  ...
  ifelse(!dir.exists(file.path(ResultDir, "PfPR")), 
         dir.create(file.path(ResultDir, "PfPR")), FALSE)
  print_path <- file.path(ResultDir, "PfPR")
  ...
  if (grepl("LGA|State|repDS|region", subVariable)) {
    ...
    table(pfpr.ls[[3]]$hml35)
    
      if(subVariable == "LGA"){
        print("computing raw PfPR coverage estimates at the LGA-level for years 2008, 2010, 2013, 2015 & 2018")
        var1 <- list("LGA")
        #var2 <- list("num_p")
        PR_LGA <- map2(pfpr.ls, var1, generate.PR.state_LGA_repDS)
        ...
        var <- list("State")
        #var2 <- list("num_p")
        PR_State <- map2(pfpr.ls,var1, generate.PR.state_LGA_repDS)
        fin_df <- plyr::ldply(PR_State, rbind)
...
# # 2018 PR NGA DHS 
# 
# pfpr.ls[[3]] <- dataclean.para(pfpr.ls[[3]], hv005, hc1, hml32, 'hml32', 'p_test') 
# pfpr.list[[3]] <- dataclean.para(pfpr.list[[3]], hv005, hc1, hml35, 'hml35', 'p_testRDT')
# 
# table(pfpr.list[[3]]$time2)
# 
# hml32.svyd18 <- svydesign.fun(pfpr.ls[[3]])
# 
...
# S_merge_ym_18 <- result.fun('p_test', 'LGA', 'num_p', design=hml32.svyd18) #microscopy 
# head(S_merge_ym_18)
# 
# 
