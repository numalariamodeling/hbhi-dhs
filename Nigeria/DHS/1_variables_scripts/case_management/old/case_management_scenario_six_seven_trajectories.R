...
wd <-getwd()
print_path <- "results/archetype_sim_input/Intervention_files_LGA/case_management/model_params"

#case management data (2010 - 2018)
cm_3 <- read.csv('bin/projection/s3_v3/cm_trend.csv') %>%  dplyr::select(year, repDS, comboACT)
hist(cm_3$comboACT)
...
sink(file.path(wd, print_path, "overall_beta_model_param.txt"))
